...
public class SELECTOR_SELECT_ENTITY_BY_NAME extends Action {
    @Override
    public Object execute(Selector selector, Object[] args) {
        Selector ret = new Selector();
        selector.getWorld().getEntities().forEach((e) -> {
            if (e.getName() == args[0].toString()) {
                ret.setSelector(new Entity[] {e});
            }
        });
        return ret;
    }
